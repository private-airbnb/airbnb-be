...
export class AppSettings {
  ...
  enabledMicroservices: boolean =
    env('ENABLED_MICROSERVICES', 'false') == 'true';
  enablesElasticsearch: boolean =
    env('ENABLED_ELASTICSEARCH', 'false') == 'true';
  ...
  static forRoot() {
    ...
    if (!AppSettings.current) {
      ...
      AppSettings.current.production =
        production == 'production' || production == 'prod'
          ? 'production'
          : production == 'staging' || production == 'stage'
          ? 'staging'
          : 'development';
      AppSettings.current.templatePath = production
...
export class Elasticsearch {
  @IsNotEmpty()
  nodes: string[] = (env('ELASTICSEARCH_NODES') || '')
    .split(';')
...
export class RabbitMQ {
   ...
   */
  getRqmTransportConfig(queueName: string): ClientProviderOptions {
    return {
      name: queueName,
